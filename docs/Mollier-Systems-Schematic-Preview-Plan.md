...
## Plant-room template merge (energy-centre assembly)

Following the review request to build on a real plant-room template
(`Plantroom-Only.json`) rather than emit an air-only energy centre, the bridge
can now seed the result from an existing `SystemEnergyCentre`:

- `Query.SystemEnergyCentre(path)` / `Query.SystemEnergyCentre_FromResources(fileName)`
  load a template energy centre from a SAM JSON file. `Plantroom-Only.json` is
  bundled under `files/resources/Analytical/Systems/SystemEnergyCentre/`.
- `Create.SystemEnergyCentre(..., SystemEnergyCentre template)` (supply-only and
  supply+extract overloads) builds the air side from the Mollier processes and
  copies each air system — with its components and routed display connections —
  into the **first plant room of a copy of the template** via the existing
  `SystemPlantRoom.CopyFrom(plantRoom, guid)` transitive copy. The template's
  liquid systems, energy sources and plant equipment are preserved untouched, and
  the supplied template object is never mutated. A `null` template degrades to the
  previous air-only behaviour.
- The Grasshopper components `SAMSystems.MollierTwinWheelExample` and
  `SAMSystems.CreateEnergyCentreByMollier` expose an optional
...
## Single air system + room-side group

Per maintainer feedback, supply and extract now form a **single** `AirSystem`
(not two). The bridge closes the air loop through the room:

```
FreshAir -> Exchanger -> Cooling -> Reheat -> SupplyFan -> Group Junction ->
Damper -> Room (Space) -> Group Junction -> Exchanger(extract) -> ExtractFan -> ExhaustAir
```

The room `Space` is built from the Mollier room condition point (dry-bulb / RH as
setpoints; area, volume and design-condition flows are placeholders). The
room-side items (Room, Damper, both Group Junctions) are wrapped in a
`DisplayAirSystemGroup` named after the air system, mirroring the reference
`airsystem-withPlantoom.json` object graph.

Still open (need a Grasshopper viewport to verify): the **two-row coordinate
layout** - supply left-to-right on the top row, extract mirrored right-to-left on
the bottom row, with connector In/Out recomputed from position. The current
converter lays the single air system on one flow-ordered row. Also open: the
second heat-recovery offset, and the `CreateAirSystemByMollier` rename with an
`AirSystem`-typed output (needs a carrier-object decision).
